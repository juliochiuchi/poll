...
import { SearchOutlined, UserOutlined, SaveOutlined, DeleteOutlined } from '@ant-design/icons';
...
const Signatures = () => {
  ...
  const [dataSource, setDataSource] = useState<IDataTypeTableSignatures[]>([]);  // estado inicial dos dados da tabela
  ...
  // configuração e execução da trigger do recurso de selecionar uma pessoa na lista | rowSelection object indicates the need for row selection
  const rowSelection = {
  ...
  // configuração do recurso de filtro da tabela na coluna 'nome' e execução do recurso
   ...
   * @name handleDelete
   * @param key 
   * @description deleta uma pessoa da lista
   * @returns retorna a lista atualizada com a remoção de uma pessoa
   */
  const handleDelete = (key: React.Key) => {
    try {
      const newData = dataSource.filter((item) => item.key !== key);
      setDataSource(newData);
      contextGlobal.onUpdateStorageSignatures(newData, 0);
   ...
   * @name handleAdd
   * @param values 
   * @description adiciona uma nova pessoa na lista
   * @returns retorna a lista atualizada com a adição de uma nova pessoa
   */
  const handleAdd = (values: any) => {
    try {
      setClickLoading(!clickLoading);
      const newData: IDataTypeTableSignatures = {
        key: count + 1,
        id: count + 1,
        name: values.completedName,
      ...
      contextGlobal.onUpdateStorageSignatures([...dataSource, newData], count + 1);
      setCount(count + 1);
      message.success('Cadastrado com sucesso!');
    } catch (e: any) {
      message.error('Erro ao adicionar!');
   ...
   * @param confirm 
   * @param dataIndex 
   * @description busca todos os resultados do filtro
   * @returns retorna todos os resultados do filtro
   */
  const handleSearch = (
    selectedKeys: string[],
   ...
   * @name handleReset
   * @param clearFilters 
   * @description reseta todas as buscas do filtro 
   * @returns reseta todas as buscas do filtro
   */
   ...
   * @name onFinish
   * @param values 
   * @description finaliza o cadastro de uma nova pessoa
   * @returns retorna o cadastro de uma nova pessoa
   */
  ...
  useEffect(() => {
    setDataSource(contextGlobal.listSignatures);
  }, [contextGlobal.listSignatures]);

  useEffect(() => {
    setCount(contextGlobal.IDKeyListSignatures);
  }, [contextGlobal.IDKeyListSignatures]);

  return (
    <div className="p-6">
      {
        !contextGlobal.loadingPageSignatures
          ? (
            <>
              <Form
              ...
              >
                <Form.Item
                  className="laptop:w-[50%] phoneTablet:w-[100%] resetMarginBottom grow"
                  name="completedName"
                  label="Nome"
                  rules={[
                    {
                ...
                <Form.Item
                  className="laptop:w-[15%] phoneTablet:w-[100%] resetMarginBottom grow"
                  name="nickname"
                  label="Nickname"
                  tooltip="Como você gostaria que os outros te chamassem?"
                  rules={[{ required: true, message: 'Digite o seu apelido!', whitespace: true }]}
                >
                  <Input />
                </Form.Item>
                ...
                <Form.Item
                  ...
                  name="age"
                  label="Idade"
                  rules={[{ required: true, message: 'Digite a sua idade!' }]}
                >
                  <InputNumber style={{ width: '100%' }} />
                </Form.Item>

                <Form.Item
                  className="laptop:w-auto phoneTablet:w-[100%] resetMarginBottom grow"
                  name="genre"
                  label="Gênero"
...

              {
                contextGlobal.flagOpenedPoll && (
                  <div className="text-center mt-10 mb-3">
                    <span>Existe uma votação pendente e os dados de assinaturas não podem ser visualizados neste momento.</span>
                  </div>
                )
              }
              ...
              <Table
                ...
                columns={columns}
                dataSource={!contextGlobal.flagOpenedPoll ? dataSource : []}
              />
            </>
          ) : (
